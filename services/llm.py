...
from config.settings import GOOGLE_API_KEY as SETTINGS_API_KEY, MODEL_NAME as SETTINGS_MODEL
...
def generate_markdown_from_prompt(
    ...
    try:
        if HAVE_NEW_GENAI and key:
            client = genai.Client(api_key=key)  # type: ignore[name-defined]
            resp = client.models.generate_content(model=model, contents=[prompt])
            return getattr(resp, "text", None), None

        if HAVE_LEGACY_GENAI and key:
            genai_legacy.configure(api_key=key)  # type: ignore[name-defined]
    ...
    except Exception as e:  # pragma: no cover - runtime API error path
        return None, str(e)
